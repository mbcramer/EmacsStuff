...
"
  ...
  (let ((buffer (current-buffer))
	...
	(when ok
	  ;; Need to add the push command if git-do-push-on-commit is set
	  (message "Still need push on commit!!!!")
...
(defun git-is-remote-p ( branch)
  "returns t if branch is a string starting with \"  remotes/\""
  (string-prefix-p "remotes/" branch))
...
(defun git-delete-branch ()
  ...
  (unless git-status (error "Not in git-status buffer."))
  (let ((branch
		 (completing-read "Delete:  "  (git-local-branches) nil nil nil nil nil)))
	;;(message branch)
	(unless (eq branch nil)
	  (when (y-or-n-p (format "Are you sure you want to delete the %s branch?" branch))
...
Branch-switch-to ---   Switch to branch
...
"
  ...
  (let ((cmd
	...
	(when (string-equal "Branch-delete"    cmd) (call-interactively 'git-delete-branch))
...
(defun git-pull ()
  ...
  (unless git-status (error "Not in git-status buffer."))
  (let ((ok nil))
	(if (y-or-n-p (format "Are you sure you want to pull? "))
		(setq ok (apply 'git-call-process-display-error  (split-string git-pull-command))))
...
(defun git-push ()
  ...
  (let ((ok nil))
	(if (y-or-n-p (format "Are you sure you want to push? "))
		(setq ok (git-call-process-display-error  "push" )))
	(when ok
	  (message "Done!")
	  (git-update-status-files))))
...
(defun git-exp-branch-to-refspec (local-branch)
  "convert local branch name to refspec"
  (unless git-status (error "Not in git-status buffer."))
  (let ((vvbranch (git-call-process-string "branch" "--list" "-vv" local-branch))
		(remote-branch ""  ))
	(setq remote-branch
		  (substring vvbranch (+ (string-match "\\[" vvbranch) 1) (string-match "\\]" vvbranch)))
	(concat local-branch ":refs/remotes/" remote-branch)))



(defun git-upstream-status (branch)
"
...
"
    ...
    (cond
        ...
        ( (not branch) (propertize "DETACHED HEAD" 'face 'git-sandbox-status-yellow))
        ( (and branch git-check-remote-repo)
          (let ((results   (apply 'git-call-process-string  "fetch" "--dry-run" "-v" () ))
			   ...
			   (if (eq results nil)
				   (when t  UnavailSt)
                   ;;(when t (message "git fetch --dry-run -v failed" ) UnavailSt)

                   ;;  the following let is results non-nil case
                   (let (  (mylist ())
                           (cb-status nil)
                           (ob-status UptodateSt)
						   bname)

                        (setq bname (if (string-match "^refs/heads/" branch) (substring branch (match-end 0)) branch))

                        (setq mylist (split-string results "\n" t))
                        (setq mylist (delete (first mylist) mylist));; the first line is the "from" line

                        (dolist (line mylist)
                                (if (string-match (concat " " bname " ") line)  ;; should match local branch name
                                    (if (string-match uptodate-str line) (setq cb-status UptodateSt) (setq cb-status CurOodSt)) ;; line is branch

                                    (when (not ( string-match uptodate-str line)) (setq ob-status OtherOodSt))))

                        ;; cb-status == NIL  error
                        ;; cb-status == OOD  OOD
                        ;; cb-status == Uptodate => ob-status
                        (cond ( (eq cb-status nil) LocalOnlySt)
                              ( (string-equal cb-status CurOodSt) CurOodSt)
                              ( t ob-status))))))))
